<template>
  <div class="container">
    ...
    <div>
      ...
      <van-swipe @change="getDefaultMsg" class="roll" :loop="true" :vertical="true" :autoplay="autoplay" :touchable="false"
        :show-indicators="false" fixed>
    ...
    <!-- 标签导航栏-->
    <div class="tag">
      <van-tabs v-model="tagIndex" animated swipeable scrollspy type="line" line-height="0px">
        <van-tab v-for="(tag,index) in category" :key="index" :to="{path:'/category/'+index}" :append="false">
          <template #title>
            <van-tag v-if="tagIndex==index" color="#f2826a" round style="width:70px" plain>{{ tag }}</van-tag>
            <van-tag v-else color="#707070" round style="width:70px" plain>{{ tag }}</van-tag>
          </template>
          <!-- 子页面插槽-->
    ...
    <Pop :show="show" :title="title" container="body" @func="closePop">
      ...
      <div slot="popContent">
        <!-- pop弹窗插槽 搜索 -->
        <div>
          <van-search class="realSearch" v-model="value" :placeholder="placeholder" @search="selectHotSpot" shape="round" />
    ...
    <!-- 底部导航 -->
    <van-tabbar v-model="active" active-color="#f2826a">
  ...
  export default Vue.extend({
    ...
    data() {
      return {
        active: 0,
        autoplay: 3000,
        category: ["js", "java", "pathon", "mysql", "nodejs", "typescript"],
        messages: [
          "大使痛批闯关美国1",
          "大使痛批闯关美国2",
          "大使痛批闯关美国3",
